...
| `EMBEDDING_OUT_OF_MEMORY` | — (trả trong `code` của §4.2) | GĐ6 tạo vector tràn bộ nhớ thiết bị (GPU/MPS/CPU) ngay cả ở lô MỘT mẩu, sau khi đã tự chia đôi lô hết mức có thể. Không phải lỗi cấu trúc tài liệu (khác `DOCUMENT_NOT_STRUCTURABLE`) và không phải lỗi không lường trước (khác `INTERNAL_ERROR`) — đây là giới hạn phần cứng đã biết trước. *Thêm 25/9, VEC-3* | 07 Mục 3.2 (`embedding_batch_size`) |
...
| **Reject** | **Chốt 24/9 (Q3):** `{ reason, actor }`, `reason` bắt buộc. Tài liệu bị **xoá khỏi vùng đệm ngay** — cả hồ sơ lẫn các mẩu đã cắt. Chỉ giữ **dấu vết**: ai từ chối (`actor.user_id`), lúc nào, lý do. **Không giữ nội dung.** Trạng thái nạp chuyển sang `rejected` kèm mã riêng cho việc Manager từ chối, khác với từ chối do định dạng. Người nộp muốn sửa thì nộp lại từ bản gốc trên MinIO. Gọi `reject` lần hai: trả kết quả như lần đầu. Nơi lưu dấu vết do work-order cài đặt đề xuất, qua `schema-guardian` |
...
| ~~Q3~~ | — | ~~Tài liệu bị Manager từ chối ở tiền kiểm~~ | **Đã chốt 24/9** — xoá khỏi vùng đệm ngay, chỉ giữ dấu vết, xem §4.4 |
