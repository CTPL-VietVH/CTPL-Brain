...
> 📌 **`schema_breaking_version` / `schema_additive_version` (SCHEMA-stamp-store, 25/9/2026, `packages/schema/embedding_registry.py`)**: bảng `embedding_model_collections` — bảng mang bản ghi ACTIVE của từng collection, KHÔNG phải bảng catalog `embedding_models` ở callout trên — khoá theo `collection_name`; con dấu THẬT của hai số phiên bản ở ràng buộc 3 dưới đây (`packages/schema/version.py`), đóng lên chính MỘT collection lúc `tools/provision/provision_stores.py` chạy, cùng nhà với `embedding_model` (không tạo bảng thứ hai). Hai cột NULLABLE, không có giá trị mặc định: một hàng có model đã active nhưng CHƯA từng qua bước đóng dấu số phiên bản schema là "kho cũ không có dấu" — một trạng thái CÓ NGHĨA, phải phân biệt được với "đã đóng dấu và khớp", không phải một mặc định ẩn lấp chỗ trống. (Kho dựng TRƯỚC khi hai cột này tồn tại bị `tools/provision/provision_stores.py` chặn SỚM HƠN, bởi phép kiểm lệch cột đã có sẵn — trạng thái NULL này xảy ra khi bản ghi active đã tồn tại nhưng riêng bước đóng dấu số phiên bản schema chưa chạy, ví dụ tiến trình provision bị cắt giữa hai bước.) Lúc khởi động (`packages/api/main.py`) và lúc tái-provision trên kho đã tồn tại, so hai cột này với `LOCAL_SCHEMA_VERSION`: lệch số PHÁ VỠ → từ chối chạy; lệch số BỔ SUNG → ghi log, vẫn chạy — đúng khuôn DX3. Cùng lý do `model_version` ở trên, đây **KHÔNG phải trường thứ tư/năm của hợp đồng mô hình**: đây là một con dấu KHÁC (số phiên bản của chính module schema, không phải mô hình biểu diễn) tình cờ sống cùng bảng vì DX2 chỉ định "cùng nhà, không tạo nhà thứ hai". Không nằm trong `ContractConfig`/`StoreStamp`, và bản thân việc thêm hai cột này KHÔNG làm tăng `SCHEMA_ADDITIVE_VERSION` — cùng lý do cột `model_version` ở trên không làm tăng: "trường" ở bảng DX3 nghĩa là trường của MỘT trong bốn thực thể dùng chung ở Mục 2 (`embedding_models`/`embedding_model_collections` không phải một trong bốn, và cũng không có mặt ở bảng kiểm kê Mục 4); hai cột này là hạ tầng ĐỌC/GHI con dấu của module schema, không phải dữ liệu hai service trao đổi. **"Trường" trong bảng DX3 là trường của một trong bốn thực thể dùng chung ở Mục 2; cột của bảng hạ tầng con dấu không tính.**
...
### 3.2 Mười ba giá trị tham số — CHỐT 14/9/2026

Mười ba con số dưới đây trước nay chỉ có **cơ chế**, không có **giá trị**, vì đều ghi là "cần đo trên dữ liệu thật". Nhưng Mục 9.6 của tài liệu 06 chốt v1 không phát sự kiện đo lường nào — nên sẽ không có dữ liệu thật để đo, và vẫn phải có người điền số.
...
| `embedding_batch_size` — số mẩu tối đa trong MỘT lệnh gọi mô hình BGE-M3 để tạo vector | **32 mẩu** | Ingestion | `EMBEDDING_OUT_OF_MEMORY` xuất hiện thường xuyên dù đã chia đôi lô nhiều lần → SIẾT. Sinh vector chậm thấy rõ mà bộ nhớ thiết bị còn dư nhiều, không lô nào tràn → NỚI |
...
> **`embedding_batch_size` chốt 25/9/2026** (work-order VEC-3-lo-vector-chong-oom), cùng đợt xử lý sự cố tràn bộ nhớ khi tạo vector cho tài liệu lớn. Trước đó con số 32 là một hằng số kỹ thuật viết cứng trong `vectorization.py`, hợp lệ lúc đó vì chỉ đổi cách gọi thư viện chứ không đổi kết quả (07 Mục 3.3 quy tắc 5 phân biệt hai loại này). VEC-3 gắn thêm cơ chế **chia đôi lô và chạy lại khi tràn bộ nhớ thiết bị** (GPU/MPS/CPU) — từ đó số này quyết định lô nào vừa hay tràn trên phần cứng thật, nên không còn là chuyện "chỉ đổi cách gọi" nữa và phải rời khỏi mã, có một nhà trong `config/ingestion.yaml` (CLAUDE.md Mục 4 quy tắc 2).
>
> Mẩu được **xếp giảm dần theo số token trước khi chia lô** — lô nặng nhất chạy trước, để tràn bộ nhớ lộ ra sớm thay vì ở lô cuối của một tài liệu dài. Lô 1 mẩu vẫn tràn thì từ chối bằng mã riêng `EMBEDDING_OUT_OF_MEMORY` (docs/10 §3.5) — không gộp vào `INTERNAL_ERROR` (đây không phải lỗi không lường trước được, mà là giới hạn phần cứng đã biết) và không gộp vào `DOCUMENT_NOT_STRUCTURABLE` (đây không phải lỗi cấu trúc tài liệu).
...
**Mười ba tham số này chia sạch theo service** — bốn cái của Retrieval, chín cái của Ingestion, không cái nào cần hai bên cùng biết. Khác hẳn cấu hình mô hình ở 3.1, vốn là **hợp đồng** mà lệch nhau là hỏng.
...
| **Ingestion** | `saturation_epsilon`, `saturation_rounds`, `scan_pair_budget`, `scan_time_budget`, `chunk_length_cap`, `max_upload_bytes`, `source_download_timeout_seconds`, `qdrant_upsert_batch_points`, `embedding_batch_size`, danh sách định dạng nhận | chỉ Ingestion | không ảnh hưởng bên kia |
...
| **Đối tượng nạp** (`ingestion_record`) *(thêm 24/9/2026; thêm cột nội bộ `promoting_document_id` 25/9/2026)* | Ingestion ghi và đọc; Retrieval không đọc | `10` Mục 4.1–4.2 — `ingestion_id` (**khác** `document_id`), `space_id`, trạng thái, người nộp, mã lỗi; **không** `url`, không dấu hiệu quyền của người gọi, không `space_is_private`, không nội dung. **Thêm `promoting_document_id` (VEC-2)**: tài liệu mà lượt nạp này *đang ghi dở* — ghi xuống TRƯỚC khi ghi hồ sơ PostgreSQL, để lần khởi động sau dọn được hồ sơ mồ côi khi tiến trình chết giữa bước ghi PG và bước ghi Qdrant. **Nội bộ, không bao giờ ra API** — cột `document_id` công khai giữ nguyên nghĩa "chỉ có giá trị khi `active`" của `10` Mục 4.2. **Thêm cột này KHÔNG tăng số phiên bản schema nào** (Mục 3.1 DX3: "trường" là trường của một trong bốn thực thể dùng chung ở Mục 2; `ingestion_record` không phải một trong bốn) | có trong mã: `packages/schema/ingestion_record.py`, DDL ở `store_schema.py` |
