...
> ⚠️ **`08` Phần E việc 1 KHÔNG ghi đường dẫn tới mã nguồn hệ cũ.** Đường dẫn dưới đây do người lập ước lượng tự xác định bằng cách rà các repo cùng cấp, **PO cần xác nhận lại trước khi tin các dòng "tín hiệu hệ cũ"** trong tài liệu này. Nếu xác nhận là sai repo thì mọi con số vẫn đứng được — chúng được dựng bottom-up từ đặc tả, hệ cũ chỉ dùng để **soát lại**, không dùng để **suy ra**.

| | |
|---|---|
| **Đường dẫn đã dùng** | `/Users/hoangvietvu/Project/Sources/LuminBrain/src/services/` |
| **Căn cứ nhận diện** | có đúng hai thư mục service tên `ingrestion` *(chính tả gốc của hệ cũ)* và `retrieval`, cộng một lõi RAG dùng chung `luminrag` |

**Quy mô đo được** (loại trừ `.venv`, `egg-info`):

| | Mã nguồn | Ca thử |
|---|---|---|
| `ingrestion/` | ~6.800 dòng | ~2.100 dòng |
| `retrieval/` | ~12.500 dòng | ~5.000 dòng |
| **Cộng** | **~19.300 dòng** | **~7.100 dòng** |

Vài module lớn, dùng làm mốc so kích thước cho các hạng mục tương ứng:

| Module hệ cũ | Dòng | Soát lại hạng mục nào ở v2 |
|---|---|---|
| `retrieval/app/core/rag_engine.py` | 1.358 | T3.2 + T3.4 + T3.5 gộp lại |
| `retrieval/app/routers/query_routes.py` | 825 | bề mặt truy vấn, phần lớn nằm ngoài phạm vi `07` Mục 0 |
| `ingrestion/app/config.py` | 755 | T1.2 (bề mặt cấu hình lớn hơn vẻ ngoài của nó) |
| `retrieval/app/config.py` | 594 | T1.2 |
| `ingrestion/app/pipeline/binary_extract.py` | 464 | T2.2 |
| `retrieval/app/core/qdrant_filter_builder.py` | 386 | T3.1 |
| `ingrestion/app/pipeline/extractor.py` | 393 | T2.2 |
| `ingrestion/app/pipeline/chunker.py` | 347 | T2.3 |
| `ingrestion/app/pipeline/merger.py` | 289 | T2.3 |
| `retrieval/app/routers/graph_routes.py` | 202 | T3.3 |
| `retrieval/app/routers/document_admin_routes.py` | 241 | T2.8 + T2.10 |
...
Đối chiếu độc lập bằng quy mô hệ cũ (Mục 2): hai service hệ cũ là ~26.400 dòng mã + test cho một phạm vi **hẹp hơn** v2 và một chuẩn test **thấp hơn** v2. 14.5–15.0 MD cho khối lượng đó là không khả thi ở bất kỳ nhịp làm việc nào. **85.5 MD** thì nhất quán với cả hai đường tính.
