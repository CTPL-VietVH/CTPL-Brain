...
| **Phạm vi** | **Chỉ hợp đồng dữ liệu giữa Ingestion v2 và Retrieval v2.** Không bao gồm hợp đồng với Backend/Frontend (Kafka, MinIO, bảng trạng thái) — đó là việc riêng khi tráo v2 vào |
...
| `labels_confirmed_by` | định danh người + thời điểm | đổi được | người đưa tài liệu vào | 5.2 GĐ5 — máy gợi ý, người xác nhận |
...
| **Lịch sử hội thoại** | Retrieval ghi, chính người dùng đọc | 06 Mục 9.4 — nguyên văn câu hỏi và câu trả lời, giữ nguyên kể cả khi quyền đổi | **chưa có** |
...
Cả ba là **việc nội bộ của Retrieval v2**, cần một tài liệu riêng. Chúng chỉ chạm tài liệu này ở một điểm: đều dùng lại `document_id` và `chunk_id` do module dùng chung định nghĩa, nên phải import từ đó chứ không tự khai báo lại.
